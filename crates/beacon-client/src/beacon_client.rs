...
impl BeaconClient {
    ...
    pub async fn subscribe_to_sse<T: serde::de::DeserializeOwned  + std::marker::Send + 'static>(
        &self,
        topic: &str,
        chan: Sender<T>,
    ) -> Result<(), BeaconClientError> {
        ...
        loop {
            ...
            while let Some(event) = es.next().await {
                let chan_local = chan.clone();
                match event {
                    Ok(reqwest_eventsource::Event::Message(message)) => {
                        match serde_json::from_str::<T>(&message.data) {
                            Ok(data) => {
                                if topic == "head" && !started {          
                                        let head_event = serde_json::from_str::<HeadEventData>(&message.data).unwrap();
                                        debug!("creatd thread of local timer");                                
                                        tokio::spawn( async move {
                                            let mut slot = head_event.slot;
                                            let slot_duration = Duration::from_secs(12);
                                            let mut slot_interval = tokio::time::interval(slot_duration);
                                            loop {
                                                slot_interval.tick().await;   
                                                debug!("slot in local {}", slot);
                                                ...
                                                slot += 1;
                                                match serde_json::from_value::<T>(synthetic) {
                                                    Ok(event) => {
                                                        if chan_local.send(event).is_err() {
                                                            debug!("no subscribers connected to synthetic event broadcaster");
                                                        }
                                                    }
                                                    Err(err) => {
                                                        error!(err=%err, "Failed to build synthetic event");
                                                    }
                                                }
                                            }                                           
                                        }) ;        
